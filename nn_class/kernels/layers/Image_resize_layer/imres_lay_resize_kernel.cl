__kernel void imres_lay_resize_kernel(
    __global const float *img_from,
    __global float *img_to,
...
)
{
    ...
    //printf("ok %lld %lld %d %d \n",i,j,convert_int_rtn(j*resize_coef_x),convert_int_rtn((j-1)*resize_coef_x));
        ...
        for(int x=j;x<last_x;x++)
            {
                ...
                img_to[channel*new_x_size*new_y_size+((int)res_y)*new_x_size+ ((int)res_x)]+=img_from[channel*old_x_size*old_y_size+y*old_x_size+x]*data_normalization_coef;

                //printf("ok %lld %lld %d \n",i,j,(int)channel*new_x_size*new_y_size+((int)res_y)*new_x_size+ ((int)res_x));

                if(resize_coef_x>1||resize_coef_y>1)
                {
                    float fill_e=img_to[channel*new_x_size*new_y_size+((int)res_y)*new_x_size+ ((int)res_x)];

                    for(int dy=(int)res_y;dy<(int)(res_y+resize_coef_y);++dy)
                        for(int dx=(int)res_x;dx<(int)(res_x+resize_coef_x);++dx)
                            img_to[channel*new_x_size*new_y_size+dy*new_x_size+ dx]=fill_e;
                }
            }
        res_y+=resize_coef_y;
    }
